        ...
        try:
            ...
            if pd.isna(bins_value):
                ...
                try:
                    ...
                    print(f"Warning: Invalid bins value for parameter '{param}': {bins_value}. Using default value 1.")
                    default_bins[param] = 1
        except Exception as e:
...
try:
    ...
    note_label = ttk.Label(scrollable_frame,
        text="Note: Tick checkbox for default values from Excel (uses bins from column 2). Use '+' to add up to 10 input fields. Input validation is based on combined range from all generations. Bins field sets bin count for each input (default from Excel, ignored for single values).",
    ...
    for i, param in enumerate(parameters):
        try:
            ...
            label = ttk.Label(param_frame, text=f"{param} (default bins: {default_bins[param]}):")
            ...
            def add_input_field(frame, row, param=param):
                try:
                    ...
                    bins_entry.insert(0, str(default_bins[param]))  # Set default bins from Excel
