...
def validate_and_generate():
    ...
    for param in parameters:
        if check_vars[param].get():
            ...
            param_inputs = []
            for entry, bins_entry, bin_name_entry, error_label in zip(entries[param], bins_entries[param], bin_name_entries[param], error_labels[param]):
                val = entry.get().strip()
                if val:
    ...
    # Apply highlighting and feedback
                ...
                for item_type, item in allowed:
                    if item_type == "range":
                        ...
                        if b == float('inf'):
                            ...
                            b_str = str(b)
                        bin_name = f"v{a}_{b_str}" if b != float('inf') else f"v{a}_inf"
                        bins.append(f"    bins {bin_name} = {{[{a}:{b_str}]}};")
                    elif item_type == "value":
                        bin_name = f"v{item}"
                        bins.append(f"    bins {bin_name} = {{{item}}};")
            elif inputs[param] and any(val.strip() for val in [e.get() for e in entries[param]]):
