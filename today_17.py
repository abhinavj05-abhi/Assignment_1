...
try:
    ...
    note_label = ttk.Label(scrollable_frame,
        ...
        wraplength=1150, foreground="blue")
        ...
        try:
            ...
            def add_cross_coverage_field(frame, row, gen=gen):
                try:
                    ...
                    coverpoints_label = ttk.Label(subframe, text="Coverpoints (comma-separated):")
                    ...
                    # Illegal bins
                    illegal_label = ttk.Label(subframe, text="Illegal bins (semicolon-separated):")
                    ...
                    help_text = ttk.Label(subframe, 
                        text="Example: cov_rtt,cov_osrmode,cov_cbw | Illegal: cov_rtt.bins1,cov_cbw.bins2; cov_osrmode.bins3,cov_cbw.bins4",
        ...
        try:
            ...
            for gen in generations:
                ...
                for name, coverpoint_list, illegal_list in cross_coverage_inputs[gen]:
                    cross_name = name.replace(' ', '_').replace('.', '_')
                    coverpoints_str = ", ".join(coverpoint_list)
                    cross_coverage = f"  {cross_name}: cross {coverpoints_str}"
                    
                    if illegal_list:
                        cross_coverage += " {\n"
                        for illegal_condition in illegal_list:
                            cross_coverage += f"    illegal_bins illegal_{illegal_condition.replace(' ', '_').replace('.', '_').replace(',', '_')} = {illegal_condition};\n"
                        cross_coverage += "  }"
