...
try:
    ...
    note_label = ttk.Label(scrollable_frame,
        text="Note: Tick checkbox for default values from Excel (uses bins from column 2). Use '+' to add up to 10 input fields. Input validation is based on combined range from all generations. Bins field sets bin count for ranges (leave empty for default array bins for single values). Configure cross coverage for each generation separately.",
        wraplength=1350, foreground="blue")
    ...
    separator.grid(row=current_row, column=0, columnspan=6, sticky="ew", pady=10)
    current_row += 1
    ...
    cross_coverage_title.grid(row=current_row, column=0, columnspan=6, pady=10)
    current_row += 1

    # Cross coverage help text
    cross_help_text = ttk.Label(scrollable_frame, 
        text="Cross Coverage Help: Enter coverpoints as 'cov_param1,cov_param2'. For illegal bins, use format: 'cov_param1.bin_name,cov_param2.bin_name; cov_param1.bin2,cov_param2.bin2'",
        wraplength=1350, foreground="gray", font=("Arial", 9))
    cross_help_text.grid(row=current_row, column=0, columnspan=6, pady=5)
    current_row += 1
    ...
    for gen in generations:
        try:
            ...
            gen_frame.grid(row=current_row, column=0, columnspan=6, sticky="ew", padx=5, pady=5)
            ...
            def add_cross_coverage_field(frame, row, gen=gen):
                try:
                    ...
                    help_text = ttk.Label(subframe, 
                        text="Example: Name='rtt_cbw_cross' | Coverpoints='cov_rtt,cov_cbw' | Illegal='cov_rtt.v1_10,cov_cbw.v20_40; cov_rtt.v50_max,cov_cbw.v160'",
    ...
    separator2.grid(row=current_row, column=0, columnspan=6, sticky="ew", pady=10)
    current_row += 1
    ...
    def validate_and_generate():
        try:
            ...
            for gen in generations:
                ...
                cross_coverages = []
                for name, coverpoint_list, illegal_list in cross_coverage_inputs[gen]:
                    cross_name = name.replace(' ', '_').replace('.', '_').replace('-', '_')
                    coverpoints_str = ", ".join(coverpoint_list)
                    cross_coverage = f"  {cross_name}: