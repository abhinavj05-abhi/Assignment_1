...
def parse_allowed_values(s):
    ...
    for part in parts:
        if part.startswith("[") and part.endswith("]") and ":" in part:
            ...
            b = int(b) if b != "$" and b != "∞" else float('inf') if b == "∞" else float('-inf')
...
note_label = ttk.Label(scrollable_frame,
    text="Note: Tick checkbox for IEEE default values (disables input). Use '+' to add input and bins fields. Bins field sets bin count for each input (default 1, ignored for single values).",
    wraplength=950, foreground="blue")
note_label.grid(row=0, column=0, columnspan=6, pady=10)

# GUI elements
param_frames = {}
entries = {}  # dict of lists of input entries
bins_entries = {}  # dict of lists of bins entries
check_vars = {}

def toggle_entry_state(param):
    """Enable/disable entry and bins fields based on checkbox state"""
    state = "disabled" if check_vars[param].get() else "normal"
    for entry in entries[param]:
        entry.configure(state=state)
    for bins_entry in bins_entries[param]:
        bins_entry.configure(state=state)
...
for i, param in enumerate(parameters):
    ...
    def add_input_field(frame, row):
        # Create a subframe for each input+bins pair
        ...
        # Plus button (only on the last row)
        if row == len(entries[param]) - 1:
            plus_btn = ttk.Button(subframe, text="+",
                                  command=lambda: add_input_field(frame, row + 1))
            plus_btn.grid(row=0, column=3, padx=5, pady=5)

    # Initial input field with bins and '+'
...
def validate_and_generate():
    ...
    for param in parameters:
        if check_vars[param].get():
            inputs[param] = None
            bins_counts[param] = [default_bins]  # Single default bins for IEEE defaults
        else:
            param_inputs = []
            param_bins = []
            for entry, bins_entry in zip(entries[param], bins_entries[param]):
                val = entry.get().strip()
                if val:
                    ...
                    if parsed is None:
                        ...
                        param_bins.append(bins_val if bins_val is not None else default_bins)
            if param_inputs:
                inputs[param] = param_inputs
                bins_counts[param] = param_bins
            else:
                inputs[param] = None
            ...
            if check_vars[param].get():
                ...
                for item_type, item in allowed:
                    if item_type == "range":
                        ...
                        if b == float('inf'):
                            b = a + 1000
                        bins.append(f"    bins bin{bin_counter}[{bins_counts[param][0]}] = {{[{a}:{b}]}};")
                        bin_counter += 1
                    elif item_type == "value":
                        bins.append(f"    bins bin{bin_counter} = {{{item}}};")
                        bin_counter += 1
            else:
                for parsed, bins_count in zip(inputs.get(param, []), bins_counts.get(param, [default_bins])):
