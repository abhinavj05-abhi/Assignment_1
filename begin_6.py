...
def get_default_bin_name(parsed, param_name):
    try:
        if parsed["type"] == "range":
            a, b = parsed["range"]
            a_st = str(a) if a != float('-inf') else "inf"
            b_st = str(b) if b != float('inf') else "$"
            a_str=a_st.replace("-", "m").replace("+", "a").replace("$","inf")
            b_str=b_st.replace("-", "m").replace("+", "a").replace("$","inf")
        ...
        if parsed["type"] == "range":
            ...
            if range_div is not None:
                ...
                while current <= max_val:
                    ...
                    current_clean = str(current).replace("-", "m").replace("+", "a").replace("$", "inf")
                    nxt_str_clean = str(next_val).replace("-", "m").replace("+", "a").replace("$", "inf")
                    bin_name_i = f"{param}_v{current_clean}_{nxt_str_clean}" if not bin_name else bin_name
...
def generate_combined_module_definition(parameters, generations, allowed_values, wifi_specs_vars, user_inputs):
    try:
        ...
        for gen in generations:
                    ...
                    if param in allowed_values[gen] and allowed_values[gen][param]:
                        ...
                        for item_type, item in allowed_values[gen][param]:
                            if item_type == "range":
                                ...
                                min_str = str(min_val) if min_val != float('-inf') else "inf"
                                max_str = str(max_val) if max_val != float('inf') else "$"
                                # Create clean names for bin identifiers (replace - with m, + with a)
                                min_str_clean = min_str.replace("-", "m").replace("+", "a")
                                max_str_clean = max_str.replace("-", "m").replace("+", "a").replace("$","inf")
                                default_bins_count = user_inputs[param]['default_bins']
                                if default_bins_count is not None and default_bins_count > 1:
                                    bin_name = f"{param.lower()}_v{min_str_clean}_{max_str_clean}"
                                    module_definition += f"        bins {bin_name}[] = {{[{min_str}:{max_str}]}};\n"
                                else:
                                    bin_name = f"{param.lower()}_v{min_str_clean}_{max_str_clean}"
                                    module_definition += f"        bins {bin_name}[] = {{[{min_str}:{max_str}]}};\n"
                            elif item_type == "value":
                                item_clean = str(item).replace("-", "m").replace("+", "a")
                                bin_name = f"{param.lower()}_v{item_clean}_{item_clean}"
            ...
            module_definition += f"{covergroup_name} u_{covergroup_name} = new();\n\n"
...
# below commands if working in linux environment
# accept excel file from command line


#if len(sys.argv)<2:
#   print(f"Usage: python3 begin_5.py <Excel file path>")
#    sys.exit(1)
...
try:
    ...
    # Mouse wheel scrolling - both vertical and horizontal
    ...
    # Set minimum width to ensure horizontal scrolling when needed
    scrollable_frame.configure(width=2000)  # Adjust this value based on your content width
