    ...
    try:
        ...
        for part in parts:
            if part.startswith("[") and part.endswith("]") and ":" in part:
                ...
                # Handle {val1,val2}==>{val3,val4} format
                left, right = part.split("==>")
                left_vals = [int(v.strip()) for v in left.strip()[1:-1].split(",") if v.strip()]
                right_vals = [int(v.strip()) for v in right.strip()[1:-1].split(",") if v.strip()]
                result.append(("transition", (tuple(left_vals), tuple(right_vals))))
            else:
                try:
                    value = int(part)
...
# Parse user input (single range, list of values, or transition)
        ...
        if s.startswith("[") and s.endswith("]") and ":" in s:
            ...
            try:
                ...
                if left_vals and right_vals:
                    return {"type": "transition", "values": (tuple(left_vals), tuple(right_vals))}
                return None
            except ValueError:
                return None
        else:
...
def get_default_bin_name(parsed, param_name):
    try:
        ...
        elif parsed["type"] == "transition":
            left_vals, right_vals = parsed["values"]
            left_str = f"{'_'.join(map(str, left_vals))}"
            right_str = f"{'_'.join(map(str, right_vals))}"
            return f"{param_name}_trans_{left_str}_to_{right_str}"
    except Exception as e:
        print(f"Error getting default bin cross_name: {e}")
        return f"{param_name}_default_bin"

# Get combined allowed range for a parameter (union of all ranges/values/transitions)
def get_combined_allowed_range(param, allowed_values):
    try:
        ...
        for left_vals, right_vals in all_transitions:
            min_vals.extend(left_vals)
            max_vals.extend(left_vals)
            min_vals.extend(right_vals)
            max_vals.extend(right_vals)
...
def get_max_allowed_range(param, allowed_values):
    try:
        ...
        if values:
            result.append(",".join(map(str, sorted(values))))
        if transitions:
            result.append(",".join(f"{{{','.join(map(str, left))}}}==>{{{','.join(map(str, right))}}}" for left, right in transitions))
        return "; ".join(result) if result else "No valid data"
    except Exception as e:
...
def is_input_within_combined_range(parsed, param, allowed_values):
    try:
        ...
        if parsed["type"] == "range":
            ...
            left_vals, right_vals = parsed["values"]
            return all(overall_min <= v <= overall_max for v in left_vals + right_vals)
        ...
        if parsed["type"] == "range":
            ...
            for item_type, item in allowed:
                if item_type == "transition":
                    allowed_left, allowed_right = item
                    if set(left_vals) == set(allowed_left) and set(right_vals) == set(allowed_right):
                        return True
            return False
        return False
    except Exception as e:
        ...
        if parsed["type"] == "range":
            ...
            if range_div is not None:
                ...
                while current <= max_val:
                    ...
                    bin_name_i = f"{param}_v{current}_{next_val}" if not bin_name else bin_name
            ...
            left_str = f"{'_'.join(map(str, left_vals))}"
            right_str = f"{'_'.join(map(str, right_vals))}"
            bin_name_v = f"{param}_trans_{left_str}_to_{right_str}" if not bin_name else bin_name
            values_str = f"{{{','.join(map(str, left_vals))}}} => {{{','.join(map(str, right_vals))}}}"
            if bins_count is not None and bins_count > 1:
                return f"        bins {bin_name_v}[{bins_count}] = {values_str};"
            else:
                return f"        bins {bin_name_v}[] = {values_str};"
...
6        for param in parameters:
        ...
        for gen in generations:
            ...
            for param in parameters:
                if param == "pkt_type":
                    continue
                # Check if osr parameter input matches Excel for coverage inclusion
                include_param = True
                if param == "osr" and param in user_inputs and not user_inputs[param]['use_default']:
                    for i, entry in enumerate(entries[param]):
                        input_text = entry.get().strip()
                        if input_text:
                            parsed = parse_user_input(input_text)
                            if parsed and parsed["type"] == "transition":
                                if not is_input_allowed_for_generation(parsed, allowed_values[gen].get(param, [])):
                                    include_param = False
                                    break
                    ...
                    if param in allowed_values[gen] and allowed_values[gen][param]:
                        ...
                        for item_type, item in allowed_values[gen][param]:
                            if item_type == "range":
                                ...
                                left_vals, right_vals = item
                                bin_name = f"{param.lower()}_trans_{'_'.join(map(str, left_vals))}_to_{'_'.join(map(str, right_vals))}"
                                values_str = f"{{{','.join(map(str, left_vals))}}} => {{{','.join(map(str, right_vals))}}}"
                                module_definition += f"        bins {bin_name}[] = {values_str};\n"
                        module_definition += "    }\n"
                else:
...
try:
    ...
    note_label = ttk.Label(scrollable_frame,
        text="Enhanced Note: Generates COMBINED functional coverage for all WiFi specifications in a single output file. Bin names default to {parameter}_v{start}_{end} or {parameter}_trans_{left}_to_{right} format. Tick checkbox for default values from Excel. Use '+' to add up to 10 input fields for parameters and cross coverage models. Range field automatically divides a range into equal bins. The code automatically adapts to any number of WiFi specification columns in the Excel file. pkt_type is automatically set from Excel and not user-configurable. Supports transition inputs like {0,1}==>{0,1} for osr parameter.",
    ...
    for i, param in enumerate(parameters):
        ...
        try:
            ...
            if combined_min is not None and combined_max is not None:
                info_text = f"Combined range: [{combined_min}:{combined_max}]"
            else:
    ...
    generate_btn = ttk.Button(scrollable_frame, text="Generate Combined Functional Coverage", 
                              command=generate_coverage)
    generate_btn.grid(row=current_row, column=0, pady=20)
    
    current_row += 1
    
    status_label = ttk.Label(scrollable_frame, text="Ready to generate combined functional coverage", 
                             font=("Arial", 10), foreground="green")
    status_label.grid(row=current_row, column=0, pady=10)
    
    instructions = """
    Instructions:
    1. Check the checkbox to use default values from Excel for a parameter
    2. When unchecked, enter custom values in the format: [min:max] for ranges, val1,val2,val3 for lists, or {val1,val2}==>{val3,val4} for transitions (e.g., {0,1}==>{0,1} for osr)
    ...
    5. Bin Name will be auto-generated if left empty in format: {parameter}_v{start}_{end} or {parameter}_trans_{left}_to_{right}
    ...
    10. pkt_type is automatically set from Excel and not user-configurable
    11. For osr, transition inputs matching Excel values (e.g., {0,1}==>{0,1}) will be included in coverage
    """
