...
# Check if all inputs for a parameter are valid across all generations
def are_all_inputs_valid_for_all_generations(param_inputs, param, allowed_values, generations):
    """
    Check if all input fields for a parameter are valid across ALL generations.
    Returns True only if every input is valid for every generation.
    """
    if not param_inputs:
        return True
    
    for parsed, _, _ in param_inputs:
        # Check if this input is valid for ALL generations
        for gen in generations:
            if not is_input_allowed(parsed, allowed_values[gen].get(param, [])):
                return False
    return True
...
# Parse allowed values with error handling
allowed_values = {}
for gen in generations:
    allowed_values[gen] = {}
    for param in parameters:
        try:
            # More robust way to access the cell value
            mask = df["Parameters"] == param
            if mask.any():
                cell_value = df.loc[mask, gen].iloc[0]
                allowed_values[gen][param] = parse_allowed_values(cell_value)
            else:
                print(f"Warning: Parameter '{param}' not found in DataFrame")
                allowed_values[gen][param] = []
        except Exception as e:
            print(f"Error parsing {param} for {gen}: {str(e)}")
            allowed_values[gen][param] = []

print("Allowed values parsed successfully")
...
note_label = ttk.Label(scrollable_frame,
    text="Note: Tick checkbox for IEEE default values (disables input). Use '+' to add up to 10 input fields. Each input field creates a separate bin. ALL input fields for a parameter must be valid across ALL generations for coverage to be generated. Bins field sets bin count for each input (default 1, ignored for single values).",
...
def validate_and_generate():
    ...
    for param in parameters:
        if check_vars[param].get():
            ...
            # Check if ALL inputs are valid for ALL generations
            if param_inputs:
                if not are_all_inputs_valid_for_all_generations(param_inputs, param, allowed_values, generations):
                    excluded_parameters.append(param)
                    # Mark all entries for this parameter as invalid
                    for entry, error_label in zip(entries[param], error_labels[param]):
                        if entry.get().strip():
                            invalid_entries.append((entry, error_label, f"Not valid across all generations"))
                else:
    ...
    if invalid_entries:
        if excluded_parameters:
            messagebox.showwarning("Validation Error", 
                f"The following parameters have inputs that are not valid across all generations and will be excluded: {', '.join(excluded_parameters)}")
        return
    ...
    for gen in generations:
        ...
        for param in parameters:
            ...
            # Skip parameters that were excluded due to invalid inputs
            ...
            if check_vars[param].get():
                ...
                if bins and param not in generated_params:
            ...
            # Case 2: User-provided inputs via GUI (each input field creates separate bins)
                ...
                for parsed, bin_count, bin_name in inputs[param]:
                    # Since we already validated that all inputs are valid for all generations,
                    # we can safely generate bins for this generation
                    if parsed["type"] == "range":
                        a, b = parsed["range"]
                        a_str = str(a)
                        b_str = "$" if b == float('inf') else str(b)
    ...
    if generated_params:
        success_msg = f"Generated coverage_model.sv successfully!\n\nParameters included: {', '.join(generated_params)}"
        if excluded_parameters:
            success_msg += f"\n\nParameters excluded (not valid across all generations): {', '.join(excluded_parameters)}"
        messagebox.showinfo("Success", success_msg)
    else:
        messagebox.showwarning("Warning", "No valid parameters found. Coverage model is empty.")
