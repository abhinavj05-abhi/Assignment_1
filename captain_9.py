...
try:
    ...
    note_label = ttk.Label(scrollable_frame,
        text="Enhanced Note: Generates COMBINED functional coverage for all WiFi specifications in a single output file. Bin names default to {parameter}_v{start}_{end} format. Tick checkbox for default values from Excel. Use '+' to add up to 10 input fields. Range field automatically divides a range into equal bins. The code automatically adapts to any number of WiFi specification columns in the Excel file. pkt_type is automatically set from Excel and not user-configurable.",
    ...
    for gen in generations:
        try:
            ...
            cross_frame.grid_columnconfigure(1, weight=1)
            cross_coverage_frames[gen] = cross_frame
            
            ttk.Label(cross_frame, text="Cross Name:").grid(row=0, column=0, sticky="w", padx=5)
            cross_name_entry = ttk.Entry(cross_frame, width=30)
            cross_name_entry.insert(0, f"cross_{gen.lower()}")
            cross_name_entry.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
            
            ttk.Label(cross_frame, text="Coverpoints (comma-separated):").grid(row=1, column=0, sticky="w", padx=5)
            coverpoints_entry = ttk.Entry(cross_frame, width=50)
            coverpoints_entry.insert(0, "")
            coverpoints_entry.grid(row=1, column=1, sticky="ew", padx=5, pady=2)
            
            ttk.Label(cross_frame, text="Illegal Bins (semicolon-separated):").grid(row=2, column=0, sticky="w", padx=5)
            illegal_entry = ttk.Entry(cross_frame, width=50)
            illegal_entry.grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        ...
        try:
            ...
            for gen in generations:
                if gen in cross_coverage_entries:
                    cross_name = cross_coverage_entries[gen]['name'].get().strip()
                    coverpoints_text = cross_coverage_entries[gen]['coverpoints'].get().strip()
                    illegal_text = cross_coverage_entries[gen]['illegal'].get().strip()
                ...
                if cross_name and coverpoints_text:
                    ...
                    if coverpoints:
                        ...
                        if gen not in user_inputs:
                            user_inputs[gen] = {}
                        if 'cross_coverage' not in user_inputs[gen]:
    ...
    6. Configure cross coverage for each WiFi specification as needed
    7. Select which WiFi specifications to include in the combined output
    8. Click Generate to create a single combined functional coverage file
    9. pkt_type is automatically set from Excel and not user-configurable
